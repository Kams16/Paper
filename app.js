...
app.put('/api/posts/:id', (req, res) => {
  ...
  try {
    ...
    if(!state) {
      ...
      // TODO: 작성자의 구독 정보를 불러온 후 푸시 알림 전송
      // 푸시 알림을 받게될 사용자 정보
      const targetUser = SimpleDatabase.select('user', {    // user 저장소에 저장되어 있는 사용자 정보 중 게시물을 작성한 사용자의 데이터 조회
        where: {
          id: updatedPost.author
        }
      });
      
      if(targetUser && targetUser.subscription) {           // 게시물 작성자의 구독 정보가 존재하는지 확인
        // TODO: 푸시 알림 보내기
        // 전송할 푸시 메시지 데이터
        const data = {
        ...
        // 구현했던 sendNotification() 사용
        sendNotification(targetUser.subscription, data).then(function(response) {
